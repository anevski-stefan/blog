...
export function TimelineClientView() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray<HTMLElement>(".timeline-item").forEach(item => {
        ...
        const nodeDot = item.querySelector(".node-dot")
        if (nodeDot) {
          ScrollTrigger.create({
            trigger: item,
            start: "top center",
            onEnter: () => nodeDot.classList.add("active"),
            onLeaveBack: () => nodeDot.classList.remove("active"),
          })
        }
      })

      gsap.utils.toArray<HTMLElement>(".year-marker").forEach(marker => {
        gsap.fromTo(
          marker,
          { scale: 0.8, opacity: 0 },
          {
  ...
  const getBadgeColor = (type: TimelineType) => {
    switch (type) {
      case "work":
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="text-sm text-home-muted font-mono hidden md:block">
              <span id="visible-count">{visibleCount}</span> milestones
            </div>
          </div>
        </div>
      </section>
        ...
        <div id="timeline-container" className="max-w-6xl mx-auto relative">
          ...
          <div className="relative">
            ...
            {timelineData.map((item, index) => {
              ...
              let showYearMarker = false
              if (index > 0) {
                if (timelineData[index - 1].year !== item.year) {
                  showYearMarker = true
                }
              }
              ...
              return (
                <div key={item.id}>
                  ...
                  >
                    <div
                      className={`order-2 ${isEven ? "md:order-1 md:text-right md:pr-16" : "md:pl-16"}`}
                    >
                      <div
                        className={`timeline-card glass rounded-2xl border border-white/10 p-6 md:p-8 relative ${item.image ? "overflow-hidden group" : ""}`}
                      >
                        <div
      ...
      <section className="py-24 px-6 md:px-16 border-t border-white/5">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <Link
              href="/#contact"
              className="inline-flex items-center gap-3 px-8 py-4 bg-home-accent hover:bg-home-accent/90 text-white font-heading text-sm font-medium tracking-wider rounded-full transition-all hover:scale-105"
            >
              Get in Touch
              <svg
                className="w-4 h-4"
            ...
            <a
              href="/resume.pdf"
              className="inline-flex items-center gap-3 px-8 py-4 border border-white/20 hover:border-home-accent/50 text-white font-heading text-sm font-medium tracking-wider rounded-full transition-all hover:bg-white/5"
            >
              <svg
                className="w-4 h-4"
                viewBox="0 0 24 24"
